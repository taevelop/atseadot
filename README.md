...
`S` 또는 장비 상점 버튼으로 모은 코인을 사용합니다. 성능 장비는 2단계까지 구매할 수 있습니다. 잔액이 부족하거나 최대 단계이면 구매되지 않으며, 확인 전 취소하면 코인을 사용하지 않습니다.
...
장비 목록 뒤에서 복장을 고를 수 있습니다. 노란 잠수복은 기본 제공되며, 검은 잠수복은 **300코인으로 한 번 구매**하면 즉시 착용합니다. 복장은 외형만 바뀌며 체력·속도·피해량에는 영향을 주지 않습니다.

| 복장 | 가격 | 착용 방식 |
| --- | --- | --- |
| 노란 잠수복 | 기본 제공 | 상점에서 선택하면 무료 착용 |
| 검은 잠수복 | 300코인, 1회 구매 | 구매 확인 후 자동 착용, 이후 무료 전환 |

상점에는 미보유 복장의 가격, 보유 중·착용 중 상태가 표시됩니다. 보유한 복장은 Z/Enter·캔버스 항목·모바일 착용 버튼으로 즉시 바꿉니다. 착용 중인 복장을 다시 선택해도 코인이 차감되지 않습니다. 구매 취소·잔액 부족 시에도 코인을 사용하지 않으며, 구매 여부와 착용 상태는 재접속·재시작·역할 전환 후에도 유지됩니다. 이전 저장은 기본 노란 복장으로 불러옵니다.
...
기존 버전에서 재고에 들어가지 않았던 포획은 최초 실행 시 한 번 복구합니다. 이전 버전에는 판매 기능이 없었으므로 누적 포획을 기준으로 일반/희귀 재고를 채우며 관찰만 가능한 생물은 제외합니다. 이후에는 판매한 수량을 재접속 때 다시 채우지 않습니다. 현재 체력은 플레이마다 초기화되며 저장 대상은 기록·재고·코인·장비·복장 보유 및 착용 상태입니다.

게임 흐름의 검증은 [PR #1 잔여 기능 검증](docs/verification.md#pr-1-잔여-기능-완성-2026-09-16), 복장 구매·착용과 화면 비교는 [잠수복 검증](docs/verification.md#검은-잠수복-구매착용-2026-09-16)에 정리했습니다.
